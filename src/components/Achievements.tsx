...
const achievements = [
  ...
  { icon: "🥇", title: "1st Place — Jan Jatiya Gaurav Diwas Speech", subtitle: "Dec 2024", color: "from-amber-100 to-amber-50" },
...
export default function Achievements() {
  ...
  return (
    <section id="achievements" className="py-32 md:py-40 px-6 md:px-12 lg:px-20 relative" ref={ref}>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 items-start">
          {/* Achievement Cards — 3 cols */}
          <div className="lg:col-span-3 grid grid-cols-1 sm:grid-cols-2 gap-4">
            {achievements.map((item, i) => (
              <motion.div
                key={i}
                className={`card-hover p-5 rounded-2xl bg-gradient-to-br ${item.color} border border-white/60 backdrop-blur-sm`}
                initial={{ opacity: 0, y: 20, scale: 0.95 }}
                animate={isInView ? { opacity: 1, y: 0, scale: 1 } : {}}
                transition={{ duration: 0.5, delay: i * 0.07 }}
              >
                <span className="text-2xl">{item.icon}</span>
                <h4 className="text-sm font-bold text-[#1a1035] mt-3 leading-snug">
                  {item.title}
                </h4>
                <p className="text-xs text-[#1a1035]/50 mt-1">{item.subtitle}</p>
              </motion.div>
            ))}
          </div>
          ...
          >
            <div className="relative aspect-square rounded-3xl overflow-hidden shadow-xl shadow-purple-100/20">
              <Image
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="relative aspect-square rounded-2xl overflow-hidden shadow-lg">
                <Image src="/images/100day_badge.png" alt="LeetCode badge" fill className="object-cover object-center" />
              </div>
              <div className="relative aspect-square rounded-2xl overflow-hidden shadow-lg">
